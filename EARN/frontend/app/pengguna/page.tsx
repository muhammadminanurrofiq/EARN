...
interface UserCardProps {
  user: User;
  index: number;
  activeDropdownIdx: number | null;
  setActiveDropdownIdx: (idx: number | null) => void;
}

// Reusable User Card Component
function UserCard({ user, index, activeDropdownIdx, setActiveDropdownIdx }: UserCardProps) {
  const panelRef = useRef<HTMLDivElement>(null);
  const isDropdownOpen = activeDropdownIdx === index;
  ...
  return (
    <div
      ref={panelRef}
      className={`glass-panel p-4 rounded-xl organic-glow flex flex-col gap-3 relative group transition-all overflow-visible ${
        isDropdownOpen ? 'z-[60]' : 'hover:z-[40]'
      }`}
    ...
    >
      <div className="absolute top-3 right-2 z-50">
        <button 
          onClick={(e) => {
            e.stopPropagation();
            setActiveDropdownIdx(isDropdownOpen ? null : index);
          }}
        ...
        {isDropdownOpen && (
          <div 
            className="dropdown-menu show rounded-xl border border-primary/20 shadow-xl py-2 bg-surface-container z-[100] shadow-2xl" 
            style={{ background: 'rgba(26, 33, 30, 0.95)', backdropFilter: 'blur(16px)', display: 'block' }}
          >
...
export default function PenggunaPage() {
  ...
  const users: User[] = [
    ...
    { initials: 'DL', name: 'Dewi Lestari', email: 'dewi.l@earn.id', avatarClass: 'bg-gradient-to-br from-primary to-tertiary text-on-primary' },
    { initials: 'EP', name: 'Eko Prasetyo', email: 'eko.p@email.com', avatarClass: 'bg-surface-container-high text-on-surface-variant' },
    { initials: 'FR', name: 'Fajri Ramadhan', email: 'fajri.r@earn.id', avatarClass: 'bg-gradient-to-br from-secondary to-primary text-on-secondary' },
    { initials: 'GP', name: 'Gita Permata', email: 'gita.p@gmail.com', avatarClass: 'bg-gradient-to-br from-tertiary to-secondary text-on-tertiary' },
  ];
  ...
  return (
    <div className="md:ml-64 flex flex-col min-h-screen relative">
      ...
      <Header
        title="Pengguna"
        subtitle="Sistem Monitoring Pengguna EARN"
        icon="group"
        rightExtra={
          <div className="relative group">
            <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant text-sm">search</span>
            <input 
              className="bg-surface-container border border-outline-variant/30 text-on-surface rounded-full pl-10 pr-4 py-2 w-64 focus:ring-1 focus:ring-primary focus:border-primary transition-all text-xs" 
      ...
      {/* Main Content Canvas */}
      <main className="flex-1 p-margin-mobile md:p-margin-desktop relative">
        ...
        <section className="grid grid-cols-1 md:grid-cols-4 gap-gutter mb-xl">
          <StatBentoCard>
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-primary font-label-sm text-label-sm bg-primary/10 px-2 py-1 rounded-full font-bold">↑ 12%</span>
            </div>
            <h3 className="font-body-sm text-body-sm text-on-surface-variant mb-1">Total Pengguna</h3>
            <p className="font-headline-lg text-headline-lg font-bold text-on-surface">3.452</p>
          ...
          <StatBentoCard>
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="font-label-sm text-label-sm text-on-surface-variant font-bold">Live</span>
              </div>
            </div>
            <h3 className="font-body-sm text-body-sm text-on-surface-variant mb-1">Pengguna Aktif</h3>
            <p className="font-headline-lg text-headline-lg font-bold text-on-surface">1.240</p>
            <p className="font-label-sm text-label-sm text-on-surface-variant mt-2">Sedang online sekarang</p>
          </StatBentoCard>
          ...
          <StatBentoCard>
            ...
            <h3 className="font-body-sm text-body-sm text-on-surface-variant mb-1">Pendaftaran Baru</h3>
            <p className="font-headline-lg text-headline-lg font-bold text-on-surface">45</p>
            <p className="font-label-sm text-label-sm text-primary mt-2 font-bold">Target tercapai hari ini</p>
          </StatBentoCard>
          ...
          <StatBentoCard>
            ...
            <h3 className="font-body-sm text-body-sm text-on-surface-variant mb-1">Total Poin Terdistribusi</h3>
            <p className="font-headline-lg text-headline-lg font-bold text-on-surface">850.200 <span className="text-label-md font-normal text-on-surface-variant">PTS</span></p>
            <div className="mt-4 flex items-center gap-2">
              ...
              <span className="font-label-sm text-label-sm text-on-surface-variant font-bold">Penerima terbanyak</span>
            </div>
          </StatBentoCard>
        </section>
        ...
        <section className="glass-panel rounded-xl border border-white/5" style={{ background: 'rgba(26, 33, 30, 0.4)' }}>
          <div className="p-md border-b border-outline-variant/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-surface-container-low">
            <h4 className="font-headline-lg-mobile text-headline-lg-mobile font-bold text-on-surface">Daftar Pengguna</h4>
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative group mr-2 hidden sm:block">
                <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant text-sm">search</span>
                <input 
                  className="bg-surface-container border border-outline-variant/30 text-on-surface rounded-lg pl-10 pr-4 py-1.5 w-48 focus:ring-1 focus:ring-primary focus:border-primary transition-all text-xs" 
                  placeholder="Cari..." 
                  type="text" 
                />
              </div>
              ...
              <button className="bg-primary text-on-primary px-4 py-2 rounded-lg font-bold text-xs flex items-center gap-2 hover:opacity-90 active:scale-95 transition-all shadow-[0_0_15px_rgba(78,222,163,0.3)]">
                <span className="material-symbols-outlined text-sm">add</span>
                Tambah Pengguna Baru
              </button>
            </div>
          </div>
          <div className="p-md overflow-y-auto" style={{ maxHeight: '400px' }}>
          ...
          <div className="px-md py-4 bg-surface-container-low border-t border-outline-variant/10 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="font-body-sm text-body-sm text-on-surface-variant">Menampilkan 8 dari 3.452 pengguna</p>
